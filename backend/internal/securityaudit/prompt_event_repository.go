...
func eventColumns(alias string) string {
	return fmt.Sprintf(`%[1]s.id,%[1]s.job_id,%[1]s.request_id,%[1]s.session_id,%[1]s.user_id,%[1]s.username_snapshot,
...
func scanEvent(row rowScanner, withFullPrompt ...bool) (*Event, error) {
	...
	dest := []any{&event.ID, &event.JobID, &event.Snapshot.RequestID, &event.Snapshot.SessionID, &userID,
