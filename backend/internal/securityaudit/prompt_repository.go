...
func insertJob(ctx context.Context, queryer sqlQueryer, snapshot PromptSnapshot, mode Mode, configVersion int64, status string, maxAttempts int) (*Job, error) {
	...
	row := queryer.QueryRowContext(ctx, `
		INSERT INTO prompt_audit_jobs (
			request_id,session_id,user_id,username_snapshot,user_email_snapshot,api_key_id,api_key_name_snapshot,
		...
		snapshot.RequestID, snapshot.SessionID, nullableID(snapshot.UserID), snapshot.UsernameSnapshot, snapshot.UserEmailSnapshot,
...
func insertEvent(ctx context.Context, queryer sqlQueryer, jobID int64, snapshot PromptSnapshot, configVersion int64, result *NormalizedResult) (*Event, error) {
	...
	row := queryer.QueryRowContext(ctx, `
		INSERT INTO prompt_audit_events (
			job_id,request_id,session_id,user_id,username_snapshot,user_email_snapshot,api_key_id,api_key_name_snapshot,
		...
		jobID, snapshot.RequestID, snapshot.SessionID, nullableID(snapshot.UserID), snapshot.UsernameSnapshot, snapshot.UserEmailSnapshot,
		nullableID(snapshot.APIKeyID), snapshot.APIKeyNameSnapshot, snapshot.GroupID, snapshot.GroupName,
		snapshot.Provider, snapshot.Endpoint, snapshot.Protocol, snapshot.Model, snapshot.PromptHash,
...
func scanJob(row rowScanner) (*Job, error) {
	...
	err := row.Scan(
		&job.ID, &job.Snapshot.RequestID, &job.Snapshot.SessionID, &userID, &job.Snapshot.UsernameSnapshot, &job.Snapshot.UserEmailSnapshot,
...
func jobColumns(alias string) string {
	return fmt.Sprintf(`%[1]s.id,%[1]s.request_id,%[1]s.session_id,%[1]s.user_id,%[1]s.username_snapshot,%[1]s.user_email_snapshot,
